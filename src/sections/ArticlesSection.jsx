        ...
        <div className="articles-grid">
          {filteredArticles.length > 0 ? (
            filteredArticles.map(article => (
              ...
              >
                <div className="article-badge">{getCategoryLabel(article.category)}</div>
                <h3>{article.title}</h3>
                <p>{article.excerpt}</p>
                ...
                <button className="read-more-btn">{t('articles.readMore')}</button>
              </article>
            ))
          ) : (
        ...
        <div className="article-modal-overlay" onClick={handleCloseModal}>
          <div className="article-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <div className="modal-badge">{getCategoryLabel(selectedArticle.category)}</div>
              <h2>{selectedArticle.title}</h2>
              <div className="modal-meta">
                <span>{t('articles.by', 'Par')} {selectedArticle.author}</span> • <span>📖 {selectedArticle.readTime} min</span>
              </div>
            </div>
            <div className="modal-body">
              {selectedArticle.fullContent.split('\n\n').map((paragraph, idx) => {
                if (paragraph.startsWith('##')) {
                  ...
                  return (
                    <ul key={idx}>
                      {items.map((item, i) => <li key={i}>{item.replace(/^- /, '').replace(/\*\*/g, '')}</li>)}
                    </ul>
                  )
                  ...
                  return (
                    <ol key={idx}>
                      {items.map((item, i) => <li key={i}>{item.replace(/^\d+\. /, '').replace(/\*\*/g, '')}</li>)}
                    </ol>
                  )
                }
